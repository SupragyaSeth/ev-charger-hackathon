...
export default function Home() {
  ...
  const [chargers, setChargers] = useState<Charger[]>([
    { id: 1, name: "Charger A", location: "Parking Lot A", isActive: true, timeRemaining: 45, currentUser: 101 },
    { id: 2, name: "Charger B", location: "Parking Lot B", isActive: false, timeRemaining: 0 },
    { id: 3, name: "Charger C", location: "Parking Lot C", isActive: true, timeRemaining: 120, currentUser: 102 },
  ]);
  ...
  function getEstimatedStartTime(position: number, chargerId: number): Date {
    ...
    const waitTime = charger.timeRemaining + (position - 1) * CHARGING_DURATION_MINUTES;
      ...
      <main className="max-w-7xl mx-auto">
              ...
              {chargers.map((charger) => (
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center">
                      <div className={`w-4 h-4 rounded-full ${getChargerStatusColor(charger)} mr-3`}></div>
            ...
            {queue.length === 0 ? (
              ...
              <div className="space-y-3">
                {queue.map((entry, index) => {
                  ...
                  const estimatedStart = getEstimatedStartTime(entry.position, entry.chargerId);
                  
                  return (
                    <div
                      key={entry.id}
                      className={`border-l-4 ${
                        index === 0 ? "border-green-500 bg-green-50 dark:bg-green-900/20" : "border-blue-500 bg-blue-50 dark:bg-blue-900/20"
                      ...
                      {index === 0 && (
                        <div className="mt-3 px-3 py-2 bg-green-100 dark:bg-green-800 rounded-lg">
                          <p className="text-sm font-medium text-green-800 dark:text-green-200">
                            🎯 Next in line! You'll be notified when your charger is available.
                          </p>
                        </div>
                      )}
                    </div>
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl p-6 text-center shadow-lg">
            ...
            <p className="text-gray-600 dark:text-gray-400 mt-1">Available Chargers</p>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl p-6 text-center shadow-lg">
            <div className="text-3xl font-bold text-blue-600 dark:text-blue-400">
              {queue.length}
            </div>
            <p className="text-gray-600 dark:text-gray-400 mt-1">People in Queue</p>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl p-6 text-center shadow-lg">
            <div className="text-3xl font-bold text-orange-600 dark:text-orange-400">
              {queue.length > 0 ? formatTime(getWaitTime(queue.length, 1)) : "0m"}
            </div>
            <p className="text-gray-600 dark:text-gray-400 mt-1">Est. Wait Time</p>
          </div>
        </div>
      </main>
    </div>
